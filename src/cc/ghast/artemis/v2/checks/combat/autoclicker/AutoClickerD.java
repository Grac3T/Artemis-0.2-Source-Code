...
extends AbstractCheck {
    ...
    public void handle(PlayerData data, NMSObject packet) {
        if (packet instanceof WrappedInArmAnimationPacket && data.combat.getLastCps() >= 35.0 && !data.user.isDigging() && !data.user.isPlaced()) {
            this.log(data, "cps=" + data.combat.getLastCps());
        }
    }
}
