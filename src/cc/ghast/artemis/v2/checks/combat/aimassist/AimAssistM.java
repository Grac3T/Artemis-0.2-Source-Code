...
extends AbstractCheck {
    ...
    public void handle(PlayerData data, NMSObject packet) {
        if (packet instanceof WrappedInFlyingPacket && ((WrappedInFlyingPacket)packet).isLook() && ((WrappedInFlyingPacket)packet).isPos()) {
            ...
            if ((double)pitchChange < 0.009 && (double)pitchChange > 0.001 && (double)pitchDifference > 1.0 && (double)yawDifference > 1.0 && (double)yawChange > 3.0 && (double)this.lastYawChange > 1.5 && (pitchChangeDifference > 1.0f || yawChangeDifference > 1.0f)) {
                this.log(data, "Experimental [2]");
            }
